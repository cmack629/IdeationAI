...
function getTimeframeRange() {
  const v = timeframeSlider.noUiSlider.get(true);
  return [Math.round(v[0]), Math.round(v[1])];
}
...
async function generateIdeas() {
  ...
  if (!apiKey) {
    setOutput("⚠️ Please enter your API key.");
    return;
  }
...
function formatOutput(text) {
  ...
  let ideaMatches = text
    .split(/Project\s*Idea\s*\d+[:\-]?\s*/i)
    .filter(s => s.trim());

  if (ideaMatches.length < 3 && text.trim()) {
    ideaMatches = [text.trim(), ...ideaMatches];
  }
